...
const ClarkIComponent = ({ activeTab, clickedArea, setClickedArea, showPopup, setShowPopup }) => {
    const [activeMap, setActiveMap] = useState(ClarkIFloor1);

    const switchToFirstFloor = () => {
        setActiveMap(ClarkIFloor1);
    };

    const switchToSecondFloor = () => {
        setActiveMap(ClarkIFloor2);
    };
